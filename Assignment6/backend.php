...
/* $sql0 = "SELECT AsWKB(EQ.SHAPE) AS wkb
FROM `earth_quakes` AS EQ, `state_borders` AS SB
WHERE CONTAINS (SB.SHAPE, EQ.SHAPE)  AND state = (SELECT state FROM `state_borders`
 WHERE CONTAINS (SHAPE, POINT({$_POST['lat']}  ,  {$_POST['lng']} )))";

  */
 
 
 
$sql1 = "SELECT AsWKB(EQ.SHAPE) AS wkb 
FROM `earth_quakes` AS EQ, `state_borders` AS SB
WHERE CONTAINS (SB.SHAPE, EQ.SHAPE)  AND state = (SELECT state FROM `state_borders` WHERE 
CONTAINS (SHAPE, POINT({$_POST['lng']}  ,  {$_POST['lat']} )))  
...
$sql2 = "SELECT state,asWKB(SHAPE) as wkb
               FROM state_borders A 
               WHERE CONTAINS(A.SHAPE,POINT({$_POST['lng']}, {$_POST['lat']}))";
